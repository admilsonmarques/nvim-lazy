...
(local mappings-view-list
  {1 {:key [:l :<CR> :o] :action :edit :mode :n}
  ...
  {:webdev_colors true
   ...
   :glyphs {:default (core.get-in icons.all [:ui :Text])
            ...
            :folder {:default (core.get-in icons.all [:ui :Fodler])
                     ...
                     :empty_open (core.get-in icons.all [:ui :EmptyFolderOpen])
                     :open (core.get-in icons.all [:ui :FolderOpen])
                     :symlink (core.get-in icons.all [:ui :FolderSymlink])}}})

(local renderer
  {:indent_markers renderer_indent_markers
...
(local filters
  {:dotfiles false :custom {1 :node_modules 2 "\\.cache"} :exclude {}})
